...
public class ActionEngine {
	...
	public void selectDropDownByIndex_custom(WebElement element, String fieldName, int ddIndex) throws Throwable {
		try {
			Select s = new Select(element);
			s.selectByIndex(ddIndex);
			ExtentFactory.getInstance().getExtent().log(Status.PASS, fieldName+"==> Dropdown Value Selected by visible text: "+ ddIndex);
		} catch (Exception e) {
			ExtentFactory.getInstance().getExtent().log(Status.FAIL, "Dropdown value not selected for field: " +fieldName +"  due to exception: "+e);
		}
	}
